...
function buildResource(
  ...
  const attrs = {
    ...
    [semconv.ATTR_SERVICE_VERSION ?? semconv.SEMRESATTRS_SERVICE_VERSION]: process.env.npm_package_version || "0.0.0",
...
type OtelSdkModule = {
  NodeSDK: new (config: { resource: unknown; traceExporter: unknown; instrumentations: unknown[] }) => {
...
export async function initTracing(serviceName: string): Promise<void> {
  ...
  try {
    ...
    const [{ NodeSDK }, autoInstr, exporter, resources, semconv] = await Promise.all([
      dynamicImport<OtelSdkModule>("@opentelemetry/sdk-node"),
      dynamicImport<OtelAutoInstrModule>("@opentelemetry/auto-instrumentations-node"),
      dynamicImport<OtelExporterModule>("@opentelemetry/exporter-trace-otlp-http"),
      dynamicImport<Parameters<typeof buildResource>[0]>("@opentelemetry/resources"),
      dynamicImport<Record<string, string>>("@opentelemetry/semantic-conventions"),
    ]);
      ...
      instrumentations: [
        autoInstr.getNodeAutoInstrumentations({
          ...
          "@opentelemetry/instrumentation-http": {
            ignoreIncomingRequestHook: (req: { url?: string }) => isOpsUrl(req.url),
          },
        }),
      ],
    });
