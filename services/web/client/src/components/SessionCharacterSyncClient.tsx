...
export default function SessionCharacterSyncClient() {
  ...
  const isCurrentUserGameMaster = useEffectEvent(() => {
    ...
    return participantsRef.current.some(
      (participant) => participant.userId === userId && participant.status === "gameMaster",
    );
  });
  ...
  useEffect(() => {
    ...
    const onSessionLaunched = (payload?: {
      ...
      if (Array.isArray(rawList)) {
        dispatch(setSessionParticipants(mapParticipantsFromSessionLaunchedPayload(rawList, code)));
      }
      scheduleRosterHttpSync();
    };
  ...
  useEffect(() => {
    ...
    const onParticipantCharacterChanged = (payload: { userId: string; characterId: string }) => {
      ...
      const next: SessionParticipant[] = exists
        ? list.map((p) => (p.userId === payload.userId ? { ...p, characterId: payload.characterId } : p))
        : [
            ...list,
            {
              id: payload.userId,
    ...
    const onSheetUpdated = ({ characterId }: { characterId: string }) => {
      ...
      if (shouldNotifyPlayerOfGmCharacterSheetUpdate(characterId, userIdRef.current, participantsRef.current)) {
    ...
    let onParticipantDisconnected: ((payload: { userId: string; username?: string }) => void) | undefined;
    let onParticipantLeft:
      | ((payload: { userId: string; username?: string; characterId?: string | null }) => void)
      | undefined;

    if (!isOnSessionLobbyPage) {
      onParticipantDisconnected = ({ userId, username }: { userId: string; username?: string }) => {
        if (userId === userIdRef.current) return;
        const list = participantsRef.current;
        const next = list.map((p) => (p.userId === userId ? { ...p, status: "disconnected" as const } : p));
        dispatch(setSessionParticipants(next));
      ...
      onParticipantLeft = (payload: { userId: string; username?: string; characterId?: string | null }) => {
        dispatch(removeSessionParticipantByUserId(payload.userId));

        const myId = userIdRef.current;
        const amGm = myId ? participantsRef.current.some((p) => p.userId === myId && p.status === "gameMaster") : false;
