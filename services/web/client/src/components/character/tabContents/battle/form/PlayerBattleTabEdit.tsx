    ...
    <div
      ...
      <Card className="gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid} orientation="vertical">
                <label htmlFor="health-temp" className="text-sm font-medium">
                  {tEdit("tempHP")}
                </label>
                <Input
                  {...field}
                  ...
                  type="number"
                />
                {fieldState.error && <FieldError id="health-temp-error" errors={[fieldState.error]} />}
              </Field>
            )}
          />
        </div>
      ...
      <Card className="gap-4 bg-red/10 border-red">
        <p className="text-sm text-gray-middle-light italic">
          🚧 <strong>{tEdit("todoForDevelopers")}</strong> - Ajouter les champs manquants pour l'onglet Combat :
          vitesses, dés de vie, jets de sauvegarde, actions, etc.
        </p>
      </Card>
    </div>
  );
