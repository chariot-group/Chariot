...
export default function MonsterPreview({ monster }: MonsterPreviewProps) {
    ...
    const parseDcFromDescription = (description?: string): { dcType?: string; dcValue?: number; successType?: string } | undefined => {
        if (!description) return undefined;
    ...
    const normalizedSavingThrows = {
        strength: Math.max(0, (monster.stats?.savingThrows?.strength ?? abilityModifier(rawAbilityScores.strength)) - abilityModifier(rawAbilityScores.strength)),
        dexterity: Math.max(0, (monster.stats?.savingThrows?.dexterity ?? abilityModifier(rawAbilityScores.dexterity)) - abilityModifier(rawAbilityScores.dexterity)),
        constitution: Math.max(0, (monster.stats?.savingThrows?.constitution ?? abilityModifier(rawAbilityScores.constitution)) - abilityModifier(rawAbilityScores.constitution)),
        intelligence: Math.max(0, (monster.stats?.savingThrows?.intelligence ?? abilityModifier(rawAbilityScores.intelligence)) - abilityModifier(rawAbilityScores.intelligence)),
        wisdom: Math.max(0, (monster.stats?.savingThrows?.wisdom ?? abilityModifier(rawAbilityScores.wisdom)) - abilityModifier(rawAbilityScores.wisdom)),
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 xl:gap-6">
                    ...
                    <div className="flex flex-col gap-2 text-sm sm:text-base xl:border-l xl:pl-6 border-border/60">
                        ...
                        <p>
                            ...
                            {baseArmorClass > 0 && computedArmorClass !== baseArmorClass ? ` (${tPreview("sourceArmorClass", { value: baseArmorClass })})` : ""}
                        </p>
                        <p>
