...
export class StripeService {
    ...
    async createCheckoutSession(dto: CheckoutDto, userId: string): Promise<IResponse<String>> {
        try {
            ...
            const product: StripeProductWithPrices = await this.findProductWithPriceById(packId);
            if (!product) {
            ...
            this.logger.debug(`Showcase URL: ${process.env.SHOWCASE_URL}`, this.SERVICE_NAME);
            ...
            const session = await this.stripe.checkout.sessions.create({
                ...
                line_items: [{
                    ...
                    price_data: {
                        ...
                        product_data: { name: `${displayName} (${product.metadata?.token_number || '0'} chars)` },
    ...
    async handleWebhook(payload: Buffer, signature: string): Promise<IResponse<boolean>> {
        try {
    ...
    private async findProductWithPriceById(productId: string): Promise<StripeProductWithPrices> {
        try {
            ...
            return productsWithPrices.find((product) => product.id === productId) || null;
        } catch (error) {
    ...
    private async fulfillOrder(session: Stripe.Checkout.Session) {
        try {
            if (!session.metadata) {
                const errorMessage: string = 'Webhook metadata required to fulfill order';
                this.logger.error(errorMessage, null, this.SERVICE_NAME);
                throw new BadRequestException(errorMessage);
            }
    ...
    private async getPurchasedQuantity(session: Stripe.Checkout.Session): Promise<number> {
        ...
        const quantity = lineItems.data.reduce((total, lineItem) => total + (lineItem.quantity || 0), 0);
